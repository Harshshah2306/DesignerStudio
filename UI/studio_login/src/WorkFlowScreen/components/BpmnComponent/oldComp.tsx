...
const BpmnComponent: React.FC<BpmnComponentProps> = ({dataset}) => {
  ...
  const [ selectedResponses, setSelectedResponses] = React.useState<Array<string>>([]);
  const [ processDefinition, setProcessDefinition] = React.useState<UIDesignerState>(dataset);
  ...
  const [ allActions] = React.useState<Array<string>>([
    'Save',
    'Deploy',
  ]);
  const [ selectedAction, setSelectedAction] = React.useState<string|null>('Save');
  ...
  const getTheUpdatedElements = ( responses: any) : any => {
    let requiredElements = {...processDefinition.elements};
    requiredElements[selectedItem].responses = responses;
    return requiredElements;
  }

  const [bpmnModelerMain, setBpmnModelerMain] = React.useState<Modeler|null>(null);
  ...
  const scanObjects = ( xmlData:Document, type:string) : Record<string,Object> => {
...
const handleSubmit = () => {
  if( selectedAction === "Save"){
    ...
    axios.put("localhost:3000/process"+processDefinition.processId, processDefinition)
    .then(res => {
        if(res.status === 200)            
        {
    ...
    .catch(err => {
        console.log("Something went Wrong! SAVING PROCESS DEFINITION" + err.message);
    });
  } else {
    let parser = new DOMParser();
    let xmlData = parser.parseFromString(processDefinition.bpmn,"application/xml");
    let requiredObject:any = getTheEvent( xmlData );

    axios.post("http://localhost:3000/process/"+processDefinition.processId, { ...processDefinition, xmlConnectors:requiredObject})
    .then(res => {
        if(res.status === 200)            
        {
            console.log("Data is being uploaded");
    ...
    <React.Fragment>
      ...
      <Fab 
        ...
        onClick = {async ()=>{
          if( bpmnModelerMain !== null){
            bpmnModelerMain.saveXML({ format: true, preamble:true }).then( res => {
              if( res.xml ){
                let parser = new DOMParser();
                let xmlData = parser.parseFromString(res.xml,"application/xml");
      ...
      <Grid container spacing={3}>
        ...
        <Grid 
          item 
          md={3} 
          style={{backgroundColor:'rgba(2,0,36,1)'}}
        >
          <Grid 
            container 
            alignItems='center'
                  ...
                  ><br/>
                    ...
                    <Grid item>
                      <TextField
                        ...
                        onChange={(evt)=> setProcessDefinition({
                          ...processDefinition,
                        ...
                        <React.Fragment>
                          {selectedItem.substring(0,5)==='Flow_'?(
                            ...
                            <Autocomplete
                              ...
                              getOptionDisabled={(value)=>selectedResponses.length>1?true:false}
                  ...
                  >
                    ...
                    <Grid item>
                      <Autocomplete
                        ...
                        onChange={(event, newValue) => setSelectedAction(newValue)}
                        sx={{ width: '250px' }}
                      />
                    </Grid>
